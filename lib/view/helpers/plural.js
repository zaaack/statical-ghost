...
plural = function (context, options) {
    if (_.isUndefined(options.hash) || _.isUndefined(options.hash.empty) ||
        ...
        return logger.logAndThrowError('All values must be defined for empty, singular and plural');
    }

    if (context === 0) {
        return new hbs.SafeString(options.hash.empty);
    } else if (context === 1) {
        return new hbs.SafeString(options.hash.singular.replace('%', context));
    } else if (context >= 2) {
        return new hbs.SafeString(options.hash.plural.replace('%', context));
    }
};
