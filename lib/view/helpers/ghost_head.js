...
function addContextMetaData(context, data, metaData) {
    // escaped data
    metaData.metaTitle = hbs.Utils.escapeExpression(metaData.metaTitle);
    metaData.metaDescription = metaData.metaDescription ? hbs.Utils.escapeExpression(metaData.metaDescription) : null;

    if (context === 'author') {
        metaData.authorUrl =  hbs.Utils.escapeExpression(blog.url + '/author/' + data.author.slug);
        ...
        metaData.authorUrl = hbs.Utils.escapeExpression(blog.url + '/author/' + data.post.author.slug);
...
function initMetaData(context, data, results) {
    var metaData = {
        url: results[0],
        metaDescription: results[1] || null,
        metaTitle: results[2],
        coverImage:  results.length > 3 ? results[3] : null,
        authorImage: results.length > 4 ? results[4] : null,
        publishedDate: null,
        modifiedDate: null,
        tags: null,
        card: 'summary',
    ...
    _.each(structuredData, function (content, property) {
        if (property === 'article:tag') {
            _.each(tags, function (tag) {
                if (tag !== '') {
                    tag = hbs.Utils.escapeExpression(tag.trim());
...
ghost_head = function (options) {
    ...
    var self = this,
        ...
        structuredData,
        schema,
        title = hbs.Utils.escapeExpression(blog.title),
    ...
    return new hbs.SafeString(headString.trim());
};
